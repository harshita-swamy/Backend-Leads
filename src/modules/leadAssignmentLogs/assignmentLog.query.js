...
export const updateLeadAssignmentQuery = async (lead_id, assigned_to) => {
...
export const insertAssignmentLogQuery = async (
  ...
  return await pool.query(
    ...
    (lead_id, assigned_to, previous_assigned_to, assigned_by, note)
    VALUES (?, ?, ?, ?, ?)`,
    [lead_id, assigned_to, previous_assigned_to, assigned_by, note]
  );
};
...
export const getAssignmentHistoryQuery = async (lead_id) => {
  const [rows] = await pool.query(
    `SELECT * FROM lead_assignment_logs
     WHERE lead_id = ?
     ORDER BY id DESC`,
    [lead_id]
  );
